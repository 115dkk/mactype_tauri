...
- `button secondary` is every other action; `secondary` is a marker class and the base `.button` rule draws it. `button danger` marks a destructive action.
...
- The `.button.danger` rule reads `--color-danger`, which no stylesheet declares, so a danger button shows the inherited text colour and a `currentColor` edge instead of `color-destructive`.
