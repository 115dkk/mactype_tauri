...
- `button secondary` is every other action; `secondary` is a marker class and the base `.button` rule draws it. `button danger` marks a destructive action: every skin sets its label in `color-destructive`, and classic and Console also draw its edge in it.
