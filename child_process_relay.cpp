...
// The relay owns only kernel handles, so it carries its own closer instead of
// a general purpose resource header.
class ScopedKernelHandle
{
public:
    explicit ScopedKernelHandle(HANDLE value) noexcept : handle_(value) {}
    ~ScopedKernelHandle()
    {
    ...
    ScopedKernelHandle(ScopedKernelHandle const&) = delete;
    ScopedKernelHandle& operator=(ScopedKernelHandle const&) = delete;
    ...
    try
    {
        ...
        if (!DuplicateHandle(
            ...
            return RelayFailure(
                ...
                GetLastError());
        ScopedKernelHandle inheritedTarget(inheritedTargetRaw);
        ...
        ScopedKernelHandle helperProcess(helper.hProcess);
        ScopedKernelHandle helperThread(helper.hThread);
