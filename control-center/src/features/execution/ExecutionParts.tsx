...
export function MigrationConfirmation({ model }: PartProps) {
  const { t } = useI18n();
  if (!model.migration.migrationConfirmationOpen) return null;
  return (
    <div className="confirmation-backdrop">
      <section
        ...
        onKeyDown={model.migration.handleMigrationDialogKeyDown}
        role="dialog"
      >
        ...
        <div className="migration-confirmation-actions">
          <button className="button secondary" onClick={model.migration.closeMigrationConfirmation} ref={model.migration.migrationCancelRef} type="button">{t("execution.migrationCancel")}</button>
          <button className="button primary" disabled={!model.service.executionView.canMigrateLegacy} onClick={() => void model.migration.confirmMigration()} type="button">{t("execution.migrationContinue")}</button>
        </div>
      </section>
    </div>
  );
...
export function LegacyTrayConflict({ model }: PartProps) {
  const { t } = useI18n();
  const { legacyTrayResolution } = model.legacy;
  ...
  return (
    <div className="legacy-tray-conflict" data-kind={legacyTrayResolution.kind} data-legacy-tray-conflict data-prominent-exception>
      ...
      <div className="legacy-tray-conflict-actions">
        <button className="button secondary" disabled={model.legacy.legacyTrayBusy !== null} onClick={() => void model.service.refresh()} type="button">
          <RefreshCw aria-hidden="true" size={16} /> {t("execution.legacyTrayCheckAgain")}
        </button>
        {legacyTrayResolution.canRequestExit && (
          <button className="button primary" disabled={model.legacy.legacyTrayBusy !== null} onClick={() => void model.legacy.exitLegacyTray()} type="button">
            <LogOut aria-hidden="true" size={16} /> {t("execution.legacyTrayExit")}
          </button>
        )}
        {legacyTrayResolution.canDisableStartup && (
          <button className="button primary" disabled={model.legacy.legacyTrayBusy !== null} onClick={() => void model.legacy.disableLegacyTrayStartup()} type="button">
...
export function ServiceSummaryNoticeAndActions({ model }: PartProps) {
  const { t } = useI18n();
  const { serviceSummary } = model.service;
  return (
    <>
      ...
      <div className="service-summary-actions">
        {serviceSummary.actions.map((action) => (
          <button
            ...
            type="button"
          >
            {model.service.serviceBusy === action.command ? t("execution.serviceWorking") : t(action.labelKey)}
          </button>
        ))}
      </div>
    </>
...
export function SystemServiceDetails({ model }: PartProps) {
  const { t } = useI18n();
  const { status, systemInjectionAction, profileIndicator, serviceStateText } = model.service;
  return (
  ...
  return (
    <div className="service-controls">
      <div>
        ...
        {executionView.serviceBinaryPath && (
          <div className="service-path">
            ...
            <button className="button secondary" onClick={() => void model.service.revealServiceLocation()} type="button">
      ...
      <div className="service-actions">
        ...
        <button className="button secondary" disabled={!executionView.canStart} onClick={() => void model.service.manageService("start")} type="button">{serviceBusy === "start" ? t("execution.serviceWorking") : t("execution.serviceStart")}</button>
        {executionView.serviceNeedsUpgrade && <button className="button secondary" disabled={!executionView.canUpgrade} onClick={() => void model.service.manageService("upgrade")} type="button">{serviceBusy === "upgrade" ? t("execution.serviceWorking") : t("execution.serviceUpgrade")}</button>}
        {executionView.serviceNeedsRepair && <button className="button secondary" disabled={!executionView.canRepair} onClick={() => void model.service.manageService("repair")} type="button">{serviceBusy === "repair" ? t("execution.serviceWorking") : t("execution.serviceRepair")}</button>}
        <button className="button secondary danger" disabled={!executionView.canRemove} onClick={() => void model.service.manageService("remove")} type="button">{serviceBusy === "remove" ? t("execution.serviceWorking") : t("execution.serviceRemove")}</button>
      </div>
    </div>
  );
}
...
export function LegacyServiceControls({ model }: PartProps) {
  ...
  return (
    <div className="service-controls legacy-service-controls" data-service-backend="legacy-mactray">
      ...
      <div className="service-actions">
        <button className="button secondary" disabled={!executionView.canMigrateLegacy} onClick={model.migration.openMigrationConfirmation} type="button">{serviceBusy === "migrate-from-legacy" ? t("execution.serviceWorking") : t("execution.migrateLegacy")}</button>
        <button className="button secondary danger" disabled={!executionView.canRemoveLegacy} onClick={() => void model.service.manageService("remove-legacy")} type="button">{serviceBusy === "remove-legacy" ? t("execution.serviceWorking") : t("execution.removeLegacy")}</button>
      </div>
    </div>
  );
}
...
export function RegisteredTargetsBody({ model }: PartProps) {
  const { t } = useI18n();
  const { status } = model.service;
  return (
    <div className="registered-launchers">
      <div className="registered-heading"><button className="button secondary" disabled={!status?.injectionReady || !status.sessionTargets.length} onClick={() => void model.targets.launchAll()} type="button"><Play aria-hidden="true" size={16} /> {t("execution.launchRegistered")}</button></div>
      {status?.sessionTargets.length ? <ul>{status.sessionTargets.map((entry) => <li key={entry.target}><code>{entry.target}</code><button aria-label={t("execution.removeTarget", { name: entry.target })} className="icon-button" onClick={() => void model.targets.remove(entry.target)} type="button"><Trash2 aria-hidden="true" size={16} /></button></li>)}</ul> : <p className="empty-state">{t("execution.noRegistered")}</p>}
    </div>
  );
}
...
export function ManualLaunchBody({ model }: PartProps) {
  const { t } = useI18n();
  const { status } = model.service;
  const { target, visibleCandidates } = model.targets;
  return (
    <>
      <div className="process-picker">
        <div className="process-picker-heading">
          ...
          <div className="process-picker-tools">
            <input aria-label={t("execution.processListFilter")} className="process-picker-filter" onChange={(event) => model.targets.setCandidateFilter(event.target.value)} placeholder={t("execution.processListFilter")} type="search" value={model.targets.candidateFilter} />
            <button className="button secondary" onClick={() => void model.targets.loadCandidates()} type="button"><RefreshCw aria-hidden="true" size={16} /> {t("execution.processListRefresh")}</button>
          </div>
        </div>
            ...
            {visibleCandidates.map((candidate) => (
              <li key={candidate.pid}>
                <label className="process-picker-row">
                  <input checked={target === candidate.path} name="manual-launch-candidate" onChange={() => model.targets.setTarget(candidate.path)} type="radio" value={candidate.path} />
      ...
      <div className="manual-launcher">
        <div className="target-picker">
          ...
          <div className="target-selection" data-empty={!target}>
            ...
            <div>
              <strong>{model.targets.targetName || t("execution.noExecutableSelected")}</strong>
              {target && <code title={target}>{target}</code>}
            </div>
            <button className="button secondary" onClick={() => void model.targets.chooseTarget()} type="button"><FolderOpen aria-hidden="true" size={17} /> {target ? t("execution.changeExecutable") : t("execution.chooseExecutable")}</button>
          </div>
        </div>
        <label><span>{t("execution.arguments")}</span><textarea onChange={(event) => model.targets.setArgumentsText(event.target.value)} placeholder={t("execution.argumentsPlaceholder")} rows={3} value={model.targets.argumentsText} /></label>
        <div className="manual-actions"><button className="button secondary" disabled={!status?.injectionReady || !target.trim()} onClick={() => void model.targets.register()} type="button"><UserPlus aria-hidden="true" size={17} /> {t("execution.register")}</button><button className="button primary" disabled={!status?.manualLauncherAvailable || !target.trim()} onClick={() => void model.targets.launch()} type="button"><Play aria-hidden="true" size={17} /> {t("execution.launch")}</button></div>
      </div>
    </>
  );
}
