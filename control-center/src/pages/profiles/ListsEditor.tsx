...
export function ListsEditor({ definitions, drafts, fontFamilies, installedFontKeys, fontOptionLabel, onDraftChange, onCommit, onUpdateFontList, t }: ListsEditorProps) {
  return (
    <div className="list-grid">{definitions.map((definition) => {
      ...
      return <label key={definition.kind}><strong><Hint content={definition.help}>{definition.label}</Hint></strong><textarea onBlur={() => onCommit(definition.kind)} onChange={(event) => onDraftChange(definition.kind, event.target.value)} rows={6} value={drafts[definition.kind] ?? ""} /></label>;
    })}</div>
  );
}
