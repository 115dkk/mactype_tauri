...
export function ClassicFiles({ onEditInTuner }: ClassicFilesProps) {
    ...
    <section className="page view-enter" aria-labelledby="files-title">
      <header className="page-header">
        ...
        <div className="header-actions">
          <button className="button" disabled={busy !== null} onClick={() => void model.files.chooseImport()} type="button"><FileInput aria-hidden="true" size={17} /> {busy === "import" ? t("files.importing") : t("files.chooseImport")}</button>
        </div>
      </header>
      ...
      {legacy && (
        <section className="legacy-import-banner" aria-labelledby="legacy-import-title">
          ...
          <button className="button primary" disabled={busy !== null} onClick={() => void model.legacy.importFrom(legacy.path)} type="button">
      ...
      <section className="section-block" aria-labelledby="profile-select-title">
        ...
        <ul className="profile-grid">
          {profiles.map((entry) => {
            ...
            return (
              <li className="profile-card" {...model.profiles.runProfileAttributes(entry)} data-selected={selected} key={entry.path}>
                ...
                <div className="profile-card-actions">
                  <button className="text-action" disabled={busy !== null} onClick={() => void model.profiles.editInTuner(entry.path)} type="button"><SlidersHorizontal aria-hidden="true" size={14} /> {t("files.editInTuner")}</button>
                </div>
              </li>
            );
          })}
      ...
      <section className="section-block" aria-labelledby="current-file-title">
        ...
        <div className="selected-file-area">
          <div className="selected-file-summary" data-empty={!profile}>
            ...
            <div><strong><CurrentFileSummary model={model} variant="editing" /></strong>{profile && <div className="selected-file-path"><CurrentFileSummary model={model} /><button aria-label={t("files.reveal")} className="icon-button" disabled={busy !== null} onClick={() => void model.files.revealCurrentProfile()} title={t("files.reveal")} type="button"><FolderOpen aria-hidden="true" size={15} /></button></div>}</div>
          </div>
        </div>
        {profile && !profile.canSave && <p className="file-save-warning">{t("files.readOnly")}</p>}
        <details className="file-details">
          <summary>{model.document.detailsSummary}</summary>
