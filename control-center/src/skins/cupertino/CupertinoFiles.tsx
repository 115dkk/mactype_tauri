    ...
    >
      ...
      <CupertinoGroup className="cupertino-profile-list" dataKind="profiles">
        {profiles.map((entry) => {
          ...
          return (
            <div className="cupertino-row cupertino-profile-row" {...model.runProfileAttributes(entry)} data-leading="true" data-selected={selected} key={entry.path}>
      ...
      <CupertinoSection title={t("files.currentTitle")}>
        <CupertinoGroup dataKind="current">
          <CupertinoRow title={t("files.editing")} value={<><CurrentFileSummary model={model} /><button aria-label={t("files.reveal")} className="button icon" disabled={!profile || busy !== null} onClick={() => void model.revealCurrentProfile()} title={t("files.reveal")} type="button"><FolderOpen aria-hidden="true" size={13} strokeWidth={1.8} /></button></>} />
