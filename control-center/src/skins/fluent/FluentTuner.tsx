          ...
          <ul className="fluent-index-list">
            {mode === "guided"
              ? editor.editing.guidedStepIds.map((step, index) => {
                ...
                return <li key={step}><button data-selected={editor.editing.activeGuidedStep === step} onClick={() => editor.editing.setActiveGuidedStep(step)} type="button"><span aria-hidden="true" className="fluent-step-num" data-done={done}>{done ? <Check aria-hidden="true" size={12} strokeWidth={2.4} /> : index + 1}</span><span>{t(`guided.${step}`)}</span></button></li>;
              })
