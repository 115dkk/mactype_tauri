...
The window is an application surface, not a page. `#root` is a flex column over a fixed-height body, the title bar and the shell grow from it, and the work area (or, in Console, the panel body) is the only element that scrolls; no shell restates the viewport height with `100vh`, and every scroll container is positioned so an absolutely placed descendant such as a screen-reader-only label cannot escape it and lengthen the document. Windows narrower than 768 px switch to document scrolling. The gallery asserts that no skin or view scrolls the document at tablet and desktop widths.
