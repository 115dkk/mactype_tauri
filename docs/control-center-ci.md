...
The profiles window smoke is stronger than a launch check: it points the app at an x86 fixture DLL with the public `IControlCenter` ABI, waits for Helper IPC plus WIC PNG output, force-terminates the child, and requires a second PNG after automatic restart before writing the ready marker.
