...
| Packaged shell process runs in an AppContainer and its loader cannot read the fixed DLL | field log of this branch, 2026-09-05: StartMenuExperienceHost, SearchHost, ShellExperienceHost, CHXSmartScreen report module-load-failed | The protected machine trees grant ALL APPLICATION PACKAGES and ALL RESTRICTED APPLICATION PACKAGES read+execute; SYSTEM and Administrators remain the only writers and the DACL stays protected. |
...
| Text stays stock until the mouse hovers it after a late injection (installer, elevated apps, children of unhooked parents) | field measurement of this branch, 2026-09-14: a GDI probe injected 1.5 s after its first paint kept the stock frame until a forced invalidation | After active admission the renderer schedules one activation repaint that waits for the DirectWrite hooks to settle and invalidates the process's visible top-level windows; `hookDirectWrite` seeds the GDI-interop render-target hooks so a process that acquired its interop before injection is covered too. Pre-entry injection has no windows yet and pays nothing. |
| Qt 6.8+ changed its default Windows text backend | [#884](https://github.com/snowie2000/mactype/issues/884), [#950](https://github.com/snowie2000/mactype/issues/950), [#1097](https://github.com/snowie2000/mactype/issues/1097) | A loaded MacType DLL is not proof of a GDI render delta. Qt 6.8 moved its default backend to DirectWrite, while Qt can also explicitly select its own FreeType backend. `SkipPrivateFreeType=1` recognizes the exact `windows:fontengine=freetype` image marker and quietly skips that process before normal injection. It does not infer a skip from a Qt module name. |
...
MacType module is present. The opt-in private-FreeType policy acts only on
explicit evidence and is repeated by the pre-entry child relay so an already
hooked parent cannot bypass it.
