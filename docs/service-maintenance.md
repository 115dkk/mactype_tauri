...
4. Compare `current.json`, `active.json`, and the DLL-adjacent `MacType.ini` according to service state. Ready requires an exact generated copy; a broker-stopped service requires that copy to be absent while the protected generation remains intact. In that stopped state, terminal health is `Unknown` with `last_error.code=runtime-profile-absent`. A recovery journal, byte mismatch, foreign runtime file, missing fixed binary, or adjacent profile present after a supported stop is a repair condition, not a cosmetic warning.
...
| one target explicitly blocks hooks/modules | Retain its exact identity and reason as a quiet process-local skip. A repeat is a duplicate; do not raise a global warning, report “performance degradation,” or blacklist the executable name. |
