...
- The solid accent fill (`.button.primary`) belongs to the action that starts or stops something. Designating the run profile writes a setting, so it wears `.button.designate` instead: accent border, accent-tinted fill, and a label deepened toward the foreground because a pure accent label falls under 4.5:1 on that tint. A skin that pins the designate fill must leave the shared `brightness` hover in place, or the button gains no hover response at all.
