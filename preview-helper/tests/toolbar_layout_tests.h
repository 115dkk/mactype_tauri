    ...
    for (bool korean : {false, true}) {
      ...
      for (std::uint32_t dpi : {96U, 144U, 192U}) {
        ...
        for (int width : widths) {
          ...
          SendMessageW(runtime.native_window_, WM_LBUTTONUP, 0, point);
          fits = fits && runtime.inverted_ == was_inverted;
          if (!fits) { std::cerr << "Toolbar layout failed: " << korean << " " << dpi << " " << width << " actual=" << client.right << " wanted=" << wanted_width << " editor=" << edit.top << " toolbar=" << runtime.toolbar_layout_height_ << '\n'; return false; }
