...
function Test-ReleaseGatePolicy {
    ...
    if ($releaseJob.If -ne "success() && github.event_name == 'push' && github.ref == 'refs/heads/main'") {
        $failures.Add('Main snapshot publication is not restricted to successful main pushes.')
    }
