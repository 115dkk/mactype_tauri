...
// The renderer applies the profile to the programs an already-rendered process
// starts, before their first instruction runs. The service only reaches them
// later, so the switch stays in front of the user and on in the default profile.
...
if (!/^HookChildProcesses\s*=\s*1\s*$/m.test(defaultProfile)) {
  throw new Error("The default profile must keep child-process propagation enabled");
}
