...
test("known legacy-selected profiles open directly without an import detour", async ({ page }, testInfo) => {
  ...
  for (const skin of gallerySkins) {
    await page.evaluate(() => window.localStorage.removeItem("mactype-control-center.recent-profile"));
    await page.goto(`/?view=profiles&gallery=1&lang=en&skin=${skin}&fresh=1&profile-runtime-missing=1`, { waitUntil: "networkidle" });
    await expect(page.locator("main code").filter({ hasText: "pretendard forever.ini" }).first()).toContainText("pretendard forever.ini");
    await page.locator('[data-nav="guided"]').click();
    await expect(page.locator(".guided-start-profile code")).toHaveText("ini\\pretendard forever.ini");
    await page.locator('[data-nav="files"]').click();
    await expect(page.locator('[data-selected="true"]').filter({ hasText: "Pretendard forever" }).first()).toBeVisible();
  }
  await page.goto("/?view=files&gallery=1&lang=en&skin=classic&fresh=1&profile-runtime-missing=1", { waitUntil: "networkidle" });
...
test("the Console skin shows the dashboard overview with a specimen, the service panel, and the activity log", async ({ page }) => {
  ...
  await expect(page.locator(".console-status")).toContainText("Control Center 서비스");
...
test("the Fluent skin renders settings cards and toggles the system expander", async ({ page }) => {
  ...
  await expect(system.locator(".fluent-sub-rows")).toContainText("프로파일 적용 상태");
...
for (const skin of gallerySkins) {
  test(`event view options hide summaries, persist, and collapse repeated failures in ${skin}`, async ({ page }) => {
    await page.goto(`/?view=diagnostics&gallery=1&lang=ko&skin=${skin}`, { waitUntil: "networkidle" });
...
test("event log sources omit absent files but report existing unreadable files", async ({ page }) => {
  ...
  await page.goto("/?view=diagnostics&gallery=1&lang=ko&skin=console", { waitUntil: "networkidle" });
  await expect(page.locator(".event-sources > div")).toHaveCount(3);
  await page.goto("/?view=diagnostics&gallery=1&lang=ko&skin=console&events-unreadable=1", { waitUntil: "networkidle" });
...
test("an event without parameters renders in the overview, the Console overview, and every diagnostics timeline", async ({ page }) => {
  ...
  for (const skin of ["classic", "fluent", "console", "cupertino"] as const) {
    ...
    await expect(page.getByTestId("event-timeline")).toContainText("서비스 시작을 마쳤습니다.");
  }
